...
class Review extends AbstractProduct
{
    ...
    public function getReviews($product)
    {
       ...
       // return $reviews[$product->id] = $product->reviews()->where('status', 'approved');
        return $reviews[$product->id] = $product->reviews()->where('status', 'pending');
    }
    ...
    public function getAverageRating($product)
    {
        ...
        return $avgRating[$product->id] = number_format(round($product->reviews()->where('status', 'pending')->avg('rating'), 2), 1);
    }
    ...
    * @param  \Webkul\Product\Contracts\Product|\Webkul\Product\Contracts\ProductFlat  $product
    ...
    public function getTotalReviews($product)
    {
        ...
        return $totalReviews[$product->id] = $product->reviews()->where('status', 'pending')->count();
    }
    ...
    public function getTotalRating($product)
    {
        ...
        return $totalRating[$product->id] = $product->reviews()->where('status', 'pending')->sum('rating');
    }
